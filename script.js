document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkAnswer(selectedAnswer) {
        ...
        if (currentQuestionIndex < quizData.length - 1) {
            ...
            // 마지막 문제일 경우 '다음 문제' 대신 최종 결과 화면으로 바로 넘어갑니다.
            // 여기서는 아무것도 하지 않고, nextButton 클릭 시 (혹은 별도의 완료 버튼 생성 시)
            // loadQuestion()에서 퀴즈 완료 처리를 합니다.
            // 따라서, 마지막 문제의 경우 정답을 선택하면 다음 버튼 대신 퀴즈 완료 화면으로 넘어갑니다.
            // 사용자가 '다음 문제' 버튼을 눌러야 결과 화면으로 넘어가는 구조를 유지하려면,
            // 이 else 문을 비워두거나, nextButton에 '퀴즈 완료' 등의 텍스트를 넣어 변경해야 합니다.
            // 현재는 nextButton을 그대로 사용하되, 클릭 시 loadQuestion()이 최종 결과를 처리하도록 합니다.
        }
    }
